...
const EditProjectModal = ({ project, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (project) {
      setForm({
        ...
        // Ensure plannedBudget is always a string
        plannedBudget: project.plannedBudget != null ? String(project.plannedBudget) : "",
  ...
  const handleChange = (e) => {
    ...
    // For numeric inputs like progress, keep it as a number
    if (name === "progress") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure plannedBudget is a string before sanitizing
      const sanitizedBudget = String(form.plannedBudget)
        .replace(/[₹,]/g, "")
        .trim();

      const payload = {
        ...form,
        plannedBudget: sanitizedBudget, // send sanitized budget
        coordinates: {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/projects/${project._id}`,
        {
          method: "PUT",
          headers: {
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50/50 sticky top-0 z-10 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-slate-900 rounded-lg shadow-sm">
              <FolderCog className="w-5 h-5 text-amber-500" />
            </div>
            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Edit Project</h2>
          </div>
          <button
            onClick={onClose}
